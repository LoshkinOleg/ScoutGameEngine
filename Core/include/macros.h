...
#define sge_CHECK_GL_ERROR() { auto errorCode = glGetError(); if (errorCode != GL_NO_ERROR){ std::cerr << "[GL_ERROR]@" << __FILE__ << "@" << __LINE__  << ": code "<< std::to_string(errorCode) << std::endl; abort(); } }
#else
